...
def fetch_feed_with_retry(url, retries=3, delay=5, code="UNK"):
    last_exception = None
    ...
    for attempt in range(retries):
        try:
            ...
            if response.status_code == 200:
                ...
                with print_lock:
                    print(f"   [{code}] Hit {response.status_code}, retrying in {wait_time:.1f}s (Attempt {attempt+1}/{retries})")
                time.sleep(wait_time)
                continue
            else:
                 with print_lock:
                    print(f"   [{code}] Unexpected status {response.status_code}")
                 return None
             ...
             if attempt < retries - 1:
                 ...
                 with print_lock:
                     print(f"   [{code}] Exception {e}, retrying in {wait_time:.1f}s")
                 time.sleep(wait_time)
             else:
                 with print_lock:
                     print(f"   [{code}] Failed after {retries} attempts: {e}")
...
def process_airport(row, keywords_dict):
    ...
    with print_lock:
        print(f"Processing {code} ({city_name}) - Country: {iso_country}, Region: {gl}...")
    ...
    with tqdm(total=len(all_phrases), desc=f"[{code}]", leave=False) as pbar:
        for category, phrase in all_phrases:
            pbar.set_description(f"[{code}] {phrase}")
            ...
            try:
                ...
                with print_lock:
                    print(f"   [{code}] Error fetching {query}: {e}")
            
            time.sleep(random.uniform(3.0, 6.0))
            pbar.update(1)

    with print_lock:
         print(f"   [{code}] Saved {len(airport_news)} unique articles.")
...
def main():
    ...
    print(f"Starting Google News scraping for {len(df_airports)} airports with SEQUENTIAL execution (using cloudscraper)...")
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        for future in concurrent.futures.as_completed(future_to_airport):
            ...
            try:
                ...
                with print_lock:
                    print(f"   [{airport_code}] generated an exception: {exc}")
