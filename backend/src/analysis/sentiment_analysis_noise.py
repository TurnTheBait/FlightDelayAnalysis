...
OUTPUT_FILE = os.path.join(backend_dir, 'data', 'sentiment', 'sentiment_scored_noise.csv')
...
for lang in keywords_config:
    if 'noise' in keywords_config[lang]:
        noise_keywords.extend(keywords_config[lang]['noise'])

noise_keywords = list(set(noise_keywords))
print(f"Loaded {len(noise_keywords)} unique noise keywords across languages.")

print(f"Reading data from: {INPUT_FILE}")
df = pd.read_csv(INPUT_FILE)

import re
pattern = '|'.join(map(re.escape, noise_keywords))
df = df[df['text'].str.contains(pattern, case=False, na=False)].copy()
print(f"Filtered for NOISE perception. Remaining rows: {len(df)}")
