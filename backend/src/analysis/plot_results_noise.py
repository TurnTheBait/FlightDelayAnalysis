...
def main():
    ...
    agg_table_path = os.path.join(OUTPUT_DIR_TAB, 'sentiment_aggregated_noise.csv')
    ...
    top_airports.to_csv(os.path.join(OUTPUT_DIR_TAB, 'top_airports_noise.csv'), index=False)
    bottom_airports.to_csv(os.path.join(OUTPUT_DIR_TAB, 'bottom_airports_noise.csv'), index=False)
    ...
    if not top_airports.empty:
        ...
        plt.title(f'Top {top_n} Airports by Noise Perception', fontsize=14, weight='bold')
    ...
    if not bottom_airports.empty:
        ...
        plt.title(f'Bottom {top_n} Airports by Noise Perception', fontsize=14, weight='bold')
        plt.xlabel('Average Score (0-10)')
        plt.ylabel('')
        plt.xlim(0, 10.5)
        plt.axvline(x=6, color='black', linestyle='--', linewidth=1, label='Neutral (6.0)')
    ...
    plt.tight_layout()
    output_ranking = os.path.join(OUTPUT_DIR_FIG, 'sentiment_ranking_top_bottom_noise.png')
    ...
    plt.title('Noise Perception: Volume vs. Sentiment', fontsize=14, weight='bold')
    ...
    output_scatter = os.path.join(OUTPUT_DIR_FIG, 'sentiment_scatter_noise.png')
    ...
    if not aggregated_scores.empty:
        ...
        plt.title('Aggregated Noise Sentiment Ranking (Grouped by Score)', fontsize=16, weight='bold')
    ...
    plt.tight_layout()
    output_aggregated = os.path.join(OUTPUT_DIR_FIG, 'sentiment_aggregated_noise.png')
