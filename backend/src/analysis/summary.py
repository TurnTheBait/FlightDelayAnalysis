...
def main():
    ...
    global_stats = df_scored.groupby('airport_code').apply(
        lambda x: pd.Series({
            ...
            'avg_pressure_impact': x['pressure_impact_score'].mean() if 'pressure_impact_score' in x.columns else np.nan
        })
    ).reset_index()

    print("Calcolo scomposizione per sorgente...")
    source_counts = df_scored.groupby(['airport_code', 'source']).size().unstack(fill_value=0)
    source_counts.columns = [f"{str(col).lower()}_count" for col in source_counts.columns]
    ...
    print(summary[['airport_code', 'total_mentions', 'global_weighted_sentiment', 'avg_pressure_impact']].head())
