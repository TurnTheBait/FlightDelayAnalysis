...
def plot_results(df, df_lagged, output_dir):
    ...
    if not df_lagged.empty:
        ...
        if not df_events.empty:
            ...
            sns.boxplot(x='Time_Lag', y='Sentiment_Score', hue='event_type', data=df_melted, palette='viridis')
            plt.title('Sentiment Score Distribution After Weather Events', fontsize=16, weight='bold')
            plt.ylabel('Average Daily Sentiment Score (1-10)', fontsize=12)
            plt.xlabel('Time Lag', fontsize=12)
            plt.axhline(y=5.5, color='gray', linestyle='--', label='Neutral Threshold (5.5)')
            plt.legend(title='Event Type', loc='best')
            plt.tight_layout()
            plt.savefig(os.path.join(output_dir, 'lagged_sentiment_boxplot.png'), dpi=300)
            plt.close()
            ...
            plt.figure(figsize=(12, 8))
            sns.lineplot(x='Time_Lag', y='Negative_Reviews', hue='event_type', data=df_melted_neg, marker='o', palette='viridis', linewidth=2, markersize=8)
            plt.title('Average Negative Reviews Count After Weather Events', fontsize=16, weight='bold')
            plt.ylabel('Avg Negative Reviews Count', fontsize=12)
            plt.xlabel('Time Lag', fontsize=12)
            plt.legend(title='Event Type', loc='best')
            plt.tight_layout()
            plt.savefig(os.path.join(output_dir, 'lagged_neg_reviews_lineplot.png'), dpi=300)
            plt.close()
        else:
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'sentiment_vs_departure_delay.png'), dpi=300)
    plt.close()
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'sentiment_vs_arrival_delay.png'), dpi=300)
    plt.close()
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'sentiment_vs_weather_prcp.png'), dpi=300)
    plt.close()
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'sentiment_vs_weather_wspd.png'), dpi=300)
    plt.close()
