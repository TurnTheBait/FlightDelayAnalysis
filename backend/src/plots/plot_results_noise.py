...
INPUT_FILE = os.path.join(backend_dir, 'data', 'sentiment', 'sentiment_results_noise.csv')
...
def main():
    ...
    total_lines = aggregated_scores['airport_code'].apply(lambda x: len(x) // 10 + 1).sum() if not aggregated_scores.empty else 10
    ...
    if not aggregated_scores.empty:
        ax_agg = sns.barplot(x='rounded_score', y='y_label', data=aggregated_scores, hue='y_label', palette=colors_agg, edgecolor='black', legend=False)
